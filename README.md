# nf-cmgg-test-datasets
Test datasets for unit tests with the nf-cmgg pipelines

## data/genomics/homo_sapiens/illumina/cram/SVcontrol
SVcontrol: subsetted crams for testing of nf-cmgg-structural
- testSV.PosCon1.cram
    sample with a homozygous deletion upstream of CHST6 (hg38.chr16:75,500,000-75,538,999). Reads subsetted chr16:75,000,000-75,600,000. Sib of PosCon2.cram.
- testSV.PosCon2.cram
    sample with a homozygous deletion upstream of CHST6 (hg38.chr16:75,500,000-75,538,999). Reads subsetted chr16:75,000,000-75,600,000. Sib of PosCon1.cram.
- testSV.PosCon3.cram
    sample with a homozygous deletion in TTLL5 (hg38.chr14:75,771,149-75,772,647). Reads subsetted chr14:75,400,000-76,000,000.
- testSV.PosCon4.cram
    sample with two heterozygous deletion on chrX with uncertain boundaries (hg38.chrX:154,097,604-154,154,739 and chrX:153,773,230-153,776,214). Reads subsetted chrX:153,400,000-154,000,000.

## data/genomics/homo_sapiens/illumina/bed/SVcontrol
SVcontrol: region of interest bed for testing of nf-cmgg-structural
- testSV.PosCon1and2.roi.bed
    bed file for chr16:74,949,851-75,608,957 with Ensembl Transcript IDs. 
- testSV.PosCon3.roi.bed
    bed file for chr14:75,294,403-76,014,143 with Ensembl Transcript IDs. 
- testSV.PosCon4.roi.bed
    bed file for chrX:153,396,962-154,014,377 with Ensembl Transcript IDs. 

## data/genomics/homo_sapiens/illumina/fastq/
Fastq files used for testing nf pipelines
- sample1_S31_R1_001.fastq.gz & sample1_S31_R1_001.fastq.gz
    This dataset of 1000 reads is generated by using region chr21:31,666,400-31,667,900 (SOD1 gene, exon 3 to 4) as template and including 2 x 3bp UMI sequences in the fastq read IDs.

## data/genomics/homo_sapiens/genome/expansionhunter
Variant catalog for ExpansionHunter and Stranger
- variant_catalog_hg38_illumina_complete.json
    Generated by running [`augment_catalog.py`](./data/genomics/homo_sapiens/genome/expansionhunter/augment_catalog.py) on an [Illumina hg38 repeat catalog](https://github.com/Illumina/RepeatCatalogs/blob/master/hg38/variant_catalog.json) and taking the Stranger annotation thresholds from the [STRchive disease loci](https://raw.githubusercontent.com/dashnowlab/STRchive/refs/heads/main/data/catalogs/STRchive-disease-loci.hg38.stranger.json).
    The script cannot resolve `PathologicRegion` for compound loci (loci with more than one `ReferenceRegion`) and warns about them:
    ```
    WARNING: 6 compound loci have no PathologicRegion. Stranger skips these:
        ATXN7
        ATXN8OS
        CNBP
        FXN
        HTT
        NOP56
    ```
    Stranger silently skips a compound locus with no `PathologicRegion`, so the six were added by hand from the [Stranger catalog](https://github.com/Clinical-Genomics/stranger/blob/main/stranger/resources/variant_catalog_hg38.json) before the file was committed.
